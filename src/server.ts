...
import type { RouteContext } from './services/callRouter.js';
...
app.post('/vapi/inbound', async (req, res) => {
  ...
  try {
    ...
    const callId: string =
      payload?.message?.call?.id ?? payload?.call?.id ?? `inbound_${Date.now()}`;
    ...
    if (!rawPhone) {
      ...
      const fallbackId =
        process.env.VAPI_ASSISTANT_FALLBACK_ID ??
    ...
    let hpResult: Awaited<ReturnType<typeof hotProspectorSearchByPhone>> | null = null;
    try {
    ...
    const vapiResponse: Record<string, unknown> = {
      assistantId: decision.assistantId,
    };
