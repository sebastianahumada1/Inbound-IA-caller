...
export class VapiWebhookHandler {
  ...
  private async handleDdpMarkTransferred(id: string, args: any, callId?: string, assistantId?: string): Promise<ToolResult> {
    try {
   ...
   */
  private async fetchLiveAgents(): Promise<{ agents: LiveAgent[]; available: LiveAgent[] }> {
    ...
    try {
      ...
      const body = await resp.json() as { agents?: LiveAgent[]; available?: LiveAgent[] };
      return {
        agents: body.agents || [],
  ...
  private async handleTransferDestinationRequest(message: any): Promise<any> {
    ...
    try {
      ...
      if (!target) {
        ...
        return {
          error:
            'No live agents are available to take the call right now. Offer to schedule a callback instead.',
        };
      }
  ...
  private async handleTranscript(message: any): Promise<WebhookResponse> {
    ...
    if (message.call?.id && message.transcript) {
      await this.stateStorage.storeTranscript(
        message.call.id,
        message.transcript,
        message.role
      );
